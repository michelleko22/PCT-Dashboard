...
# ── dispensing booth snapshot ────────────────────────────────────────────────
DISPENSING_BOOTHS = [
    ('Disp 1', 'D1', 'Dispenser 1'),
    ('Disp 2', 'D2', 'Dispenser 2'),
    ('Disp 3', 'D3', 'Dispenser 3'),
    ('Disp 4', 'D4', 'Dispenser 4'),
]

dispensing_booth_map = {b[0]: [] for b in DISPENSING_BOOTHS}
for r in all_mfg:
    if r['wc'] in dispensing_booth_map and (r['start'] or r['finish']):
        dispensing_booth_map[r['wc']].append(r)
for v in dispensing_booth_map.values():
    v.sort(key=lambda x: x['start'] or x['finish'] or datetime.min)

dispensing_booths = []
for bname, short, machine in DISPENSING_BOOTHS:
    jobs = dispensing_booth_map[bname]
    ...
    dispensing_booths.append({
        'name': bname, 'short': short, 'machine': machine,
        'status': status,
        'idle_h': idle_h,
        'current': make_disp_job(current) if current else None,
        'queue_items': [make_disp_job(j) for j in queue_jobs],
        'queue_total_h': queue_total_h,
    })
...
# ── SG med dispensing booth snapshot ────────────────────────────────────────────
MED_DISP_BOOTHS = [
    ('SG Med Disp 1', 'M1', 'Med Dispenser 1'),
    ('SG Med Disp 2', 'M2', 'Med Dispenser 2'),
    ('SG Med Disp 3', 'M3', 'Med Dispenser 3'),
]

med_dispensing_booth_map = {b[0]: [] for b in MED_DISP_BOOTHS}
for r in all_mfg:
    if r['wc'] in med_dispensing_booth_map and (r['start'] or r['finish']):
        med_dispensing_booth_map[r['wc']].append(r)
for v in med_dispensing_booth_map.values():
    v.sort(key=lambda x: x['start'] or x['finish'] or datetime.min)

med_dispensing_booths = []
for bname, short, machine in MED_DISP_BOOTHS:
    jobs = med_dispensing_booth_map[bname]
    ...
    if running:
        status_m, current_m = 'RUN', running
    elif last_done:
        status_m, current_m = 'IDLE', last_done
    elif next_sched:
        status_m, current_m = 'SCHED', next_sched
    else:
        status_m, current_m = 'IDLE', None

    idle_h = None
    if status_m == 'IDLE' and current_m and current_m.get('finish'):
        idle_h = round((SNAPSHOT_DT - current_m['finish']).total_seconds() / 3600, 1)

    def make_med_disp_job(j):
        return {
            'wo':       j['wo'],
            'item':     j['item'],
            'desc':     (j['desc'] or '').strip()[:40],
    ...
    med_dispensing_booths.append({
        'name': bname, 'short': short, 'machine': machine,
        'status': status_m,
        'idle_h': idle_h,
        'current': make_med_disp_job(current_m) if current_m else None,
        'queue_items': [make_med_disp_job(j) for j in queue_jobs],
        'queue_total_h': queue_total_h,
    })
    print(f"  med disp {short}: {status_m}, {len(jobs)} jobs, idle {idle_h}h")

# ── SG encapsulation booth snapshot ───────────────────────────────────────────
ENCAP_BOOTHS = [
    ('SG Encap 1', 'E1', 'Softgel Encap 1'),
    ('SG Encap 2', 'E2', 'Softgel Encap 2'),
    ('SG Encap 3', 'E3', 'Softgel Encap 3'),
]

sg_encap_booth_map = {b[0]: [] for b in ENCAP_BOOTHS}
for r in all_mfg:
    if r['wc'] in sg_encap_booth_map and (r['start'] or r['finish']):
        sg_encap_booth_map[r['wc']].append(r)
for v in sg_encap_booth_map.values():
    v.sort(key=lambda x: x['start'] or x['finish'] or datetime.min)

sg_encap_booths = []
for bname, short, machine in ENCAP_BOOTHS:
    jobs = sg_encap_booth_map[bname]
    ...
    if running:
        status_e, current_e = 'RUN', running
    elif last_done:
        status_e, current_e = 'IDLE', last_done
    elif next_sched:
        status_e, current_e = 'SCHED', next_sched
    else:
        status_e, current_e = 'IDLE', None

    idle_h = None
    if status_e == 'IDLE' and current_e and current_e.get('finish'):
        idle_h = round((SNAPSHOT_DT - current_e['finish']).total_seconds() / 3600, 1)

    def make_encap_job(j):
        return {
            'wo':       j['wo'],
            'item':     j['item'],
            'desc':     (j['desc'] or '').strip()[:40],
    ...
    sg_encap_booths.append({
        'name': bname, 'short': short, 'machine': machine,
        'status': status_e,
        'idle_h': idle_h,
        'current': make_encap_job(current_e) if current_e else None,
        'queue_items': [make_encap_job(j) for j in queue_jobs],
        'queue_total_h': queue_total_h,
    })
    print(f"  sg encap {short}: {status_e}, {len(jobs)} jobs, idle {idle_h}h")
...
dashboard_data = {
    ...
    'med_dispensing_booths': med_dispensing_booths,
    'sg_encap_booths': sg_encap_booths,
